...
@onready var target: CharacterBody2D = get_tree().get_first_node_in_group('right_ship_target')
@onready var smoke_and_ember = [$RightShipShootingAnimation/SmokeParticles, $RightShipShootingAnimation/EmberParticles]
...
#	$RightShipSprite.start_shake()
	$RightShipShootingAnimation.start_shake()
	var rotation_tween = create_tween()
	rotation_tween.tween_property(self, "global_rotation", 0, 0.5)
	rotation_tween.finished.connect(set_charge_state)
